...
- Detailfenster-Erkennung nutzt jetzt die vollständigen Schlüsselfrasen (`Sell`, `Set Price`, `Register Quantity`, `Total Price` bzw. `Purchase`, `Desired Price`, `Desired Amount`, `Total Cost`) mit OCR-toleranter Normalisierung. Mindestens drei Treffer inkl. `Sell`/`Purchase` genügen, sodass selbst “Se11/Pr1ce”-Varianten korrekt als Item-Dialog erkannt und vom Overview-Parser ausgeschlossen werden. Legacy-Heuristiken (Base/Min/Max) bleiben als Fallback aktiv.
